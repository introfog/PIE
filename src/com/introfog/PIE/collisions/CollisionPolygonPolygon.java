...
public class CollisionPolygonPolygon implements CollisionCallback{
	...
	private void searchContactVertices (Polygon prey, Polygon provocative, Vector2f[] contactVertices){
		...
		for (int i = 0; i < provocative.vertexCount; i++){
			tmpV.set (provocative.body.position); //преобразовали координаты вершины провокатора в координаты жертвы
			tmpV.add (provocative.vertices[i]);
	...
	private void searchNormalAndPenetration (Polygon prey, boolean reverse){
		...
		for (int i = 0; i < contactCounter; i++){
			for (int j = 0; j < prey.vertexCount; j++){
				tmpV.set (prey.vertices[(j + 1) % prey.vertexCount]); //поулчили координты ребра
				tmpV.sub (prey.vertices[j]);
			...
			prey.rotateMatrix.mul (contactVertices[i],
								   contactVertices[i]); //восстанавливаем координаты точки касания в мировые кординаты
			contactVertices[i].add (prey.body.position);
			manifold.contacts[i].set (contactVertices[i]);
		}
	...
	public void handleCollisionFromOtherProject (Manifold manifold){
		Polygon A = manifold.polygonA;
		Polygon B = manifold.polygonB;
		...
		Vector2f sidePlaneNormal = Vector2f.sub (v2, v2);
		sidePlaneNormal.normalize ();
